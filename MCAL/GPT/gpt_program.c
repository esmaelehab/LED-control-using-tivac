 ...
 *  -----------------------------------------------------------------------------------------------------------------*/
/**        \file  gpt_program.c
 *        \brief  set each timer with the required configuration
 *
 *      \details  this driver allow the user to start predefined timers and configure different timer with 
 *                different configuration with different frequincies for each one start rach timer from required tick
 *                and enable or disable each timer interrupt
 *********************************************************************************************************************/
...
/* array to hold the timers configuration ( split or concatenate ) for each timer */
static uint8 g_gptTimersMode[NUM_OF_GPT];                                            

/* array of pointers to hold the call back function addresss for each timer */
static volatile Gpt_NotificationType g_gptChannelNotification[NUM_OF_GPT] = {NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR, 
...
void GPT_init (const Gpt_ConfigType* ConfigPtr)
{
    ...
    for (i=0; i < NUM_OF_ACTIVATED_CHANNEL; i++)
    {
        ...
        /* -disable timer while initializating 
         *  -configure timer mode ( split or concatenate )
         *  -choose count down 
         */
        ...
        /* set prescaler only in split timer mode */
...
{
    SET_BIT(GPTMICR(GPT_16_32_BIT_TIMER4),0);
    (*g_gptChannelNotification[GPT_16_32_BIT_TIMER4])();
}

void TIMER5A_Handler (void)
{
    SET_BIT(GPTMICR(GPT_16_32_BIT_TIMER5),0);
    (*g_gptChannelNotification[GPT_16_32_BIT_TIMER5])();
}

void WTIMER0A_Handler (void)
{
    SET_BIT(GPTMICR(GPT_32_64_BIT_WIDE_TIMER0),0);
    (*g_gptChannelNotification[GPT_32_64_BIT_WIDE_TIMER0])();
}

void WTIMER1A_Handler (void)
{
    SET_BIT(GPTMICR(GPT_32_64_BIT_WIDE_TIMER1),0);
    (*g_gptChannelNotification[GPT_32_64_BIT_WIDE_TIMER1])();
}

void WTIMER2A_Handler (void)
{
    SET_BIT(GPTMICR(GPT_32_64_BIT_WIDE_TIMER2),0);
    (*g_gptChannelNotification[GPT_32_64_BIT_WIDE_TIMER2])();
}

void WTIMER3A_Handler (void)
{
    SET_BIT(GPTMICR(GPT_32_64_BIT_WIDE_TIMER3),0);
    (*g_gptChannelNotification[GPT_32_64_BIT_WIDE_TIMER3])();
}

void WTIMER4A_Handler (void)
{
    SET_BIT(GPTMICR(GPT_32_64_BIT_WIDE_TIMER4),0);
    (*g_gptChannelNotification[GPT_32_64_BIT_WIDE_TIMER4])();
}

void WTIMER5A_Handler (void)
{
    SET_BIT(GPTMICR(GPT_32_64_BIT_WIDE_TIMER5),0);
    (*g_gptChannelNotification[GPT_32_64_BIT_WIDE_TIMER5])();
}
